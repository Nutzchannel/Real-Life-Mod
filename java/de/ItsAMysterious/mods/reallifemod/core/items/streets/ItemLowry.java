package de.ItsAMysterious.mods.reallifemod.core.items.streets;
...
public class ItemLowry extends ExtendedItem implements Buyable{
	...
	public ItemLowry(){
		setUnlocalizedName("truck");
		setTextureName("reallifemod:MAN");
		this.setPrice(120000F);
	}
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    EntityTruck truck = new EntityTruck(world, i + 0.5F, j + 1.0F, k + 0.5F);

                    if (!world.getCollidingBoundingBoxes(truck, truck.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemStack;
                    }
                    ...
                    if (!world.isRemote)
                    {
                        world.spawnEntityInWorld(truck);
                    }
