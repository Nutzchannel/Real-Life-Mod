...
public class itemEmergencyHelicopter extends ExtendedItem{
	...
	public itemEmergencyHelicopter(){
		setUnlocalizedName("emergencyHelicopter");
		setTextureName("reallifemod:emergencyHelicopter");
	}
	@Override
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                   ...
                   /** SAR71Entity sar = new SAR71Entity(world, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    sar.rotationYaw = (float)(((MathHelper.floor_double((double)(player.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!world.getCollidingBoundingBoxes(sar, sar.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemStack;
                    }
